...
from models import render_trajectory


@torch.no_grad()
def log_gradient_health(model, x, target_image, step, rel=0.002, n_seqs=8, prefix="graddiag"):
    """
    ...
    """
    x, target_image = x[:n_seqs], target_image[:n_seqs]
    ...
    def task_loss():
        out = model(x)                                  # (b, T, 3) inference, no grad written
        pred = render_trajectory(out, model.img_H, model.img_W, model.render_sigma)
        return 0.5 * ((pred - target_image) ** 2).mean().item()
    ...
    try:
        ...
        for p in model.parameters():
            p.grad = None
        model(x, target_image=target_image, log_step=0) # e-prop writes .grad in-place
